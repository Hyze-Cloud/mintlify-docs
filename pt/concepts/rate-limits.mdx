...
| **API Hyze** | Chamadas a `api.hyze.cloud` | Deploy, logs, keys, automação |
...
Alguns endpoints da API (`api.hyze.cloud`) também têm rate limit e retornam `429` com `Retry-After`.
